...
void load_fc_kernel(quad_tw *kernel_l, quad_tw *kernel_m, quad_tw *kernel_r, data_tw k_buf_l[2][fc_in_ch / 2][fc_out_ch], data_tw k_buf_m[2][fc_in_ch / 2][fc_out_ch], data_tw k_buf_r[2][fc_in_ch / 2][fc_out_ch]){
#pragma HLS INLINE off

	for(int in = 0; in < fc_in_ch/2; ++in){
	#pragma HLS PIPELINE
		k_buf_l[0][in][0] = kernel_l[in].a;
		k_buf_l[0][in][1] = kernel_l[in].b;
		k_buf_l[1][in][0] = kernel_l[in].c;
		k_buf_l[1][in][1] = kernel_l[in].d;
	}

	for(int in = 0; in < fc_in_ch/2; ++in){
	#pragma HLS PIPELINE
		k_buf_m[0][in][0] = kernel_m[in].a;
		k_buf_m[0][in][1] = kernel_m[in].b;
		k_buf_m[1][in][0] = kernel_m[in].c;
		k_buf_m[1][in][1] = kernel_m[in].d;
	}

	for(int in = 0; in < fc_in_ch/2; ++in){
	#pragma HLS PIPELINE
		k_buf_r[0][in][0] = kernel_r[in].a;
		k_buf_r[0][in][1] = kernel_r[in].b;
		k_buf_r[1][in][0] = kernel_r[in].c;
		k_buf_r[1][in][1] = kernel_r[in].d;
	}
}
...
#pragma HLS array_partition variable=k_buf_r complete dim=3
	...
	load_fc_kernel(kernel_l, kernel_m, kernel_r, k_buf_l, k_buf_m, k_buf_r);
